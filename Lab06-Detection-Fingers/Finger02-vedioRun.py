import cv2
################# สามารถเปลี่ยนชื่อได้ ตาม File ในตัวอย่างคือ Finger01_MCU.py  ########################
import Finger01_MCU as cnt
...
while True:
    ...
    if hands:
        lmList=hands[0]
        fingerUp=detector.fingersUp(lmList)                     ###### Detector and then keep status to fingerUp[x,x,x,x]
    ...
    k=cv2.waitKey(1) & 0xFF  # Add "& 0xFF" for compatibility
    #if k==ord("k"):
    if k == 27:   #   27 press esc or change to    ord("c")   for press c letter
        break
